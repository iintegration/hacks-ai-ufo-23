...
def train_test_dataset(dir_dataset, name_xlsx):
    ...
    df_ksg = df_ksg.fillna(
        {'date_start_bpo': df_ksg.groupby(['obj_key', 'task_key'])['date_start_bpo'].transform('first')})
    ...
    for date_name1, date_name2 in date_pairs.items():
        df_ksg.loc[:, f'diff_{date_name1[5:]}_{date_name2[5:]}'] = (
                    df_ksg[date_name1] - df_ksg[date_name2]).dt.days.astype(float)
...
def analyze_data(object_name: str) -> None:
    ...
    data_train = data_train.drop(
        columns=['date_start_task', 'date_end_task', 'date_start_bpo', 'date_end_bpo', 'date_report'])
    ...
    features_train[list(features_train.select_dtypes('object').columns)] = features_train[
        list(features_train.select_dtypes('object').columns)].fillna('Пропущенное значение')

    data_predict[list(data_predict.select_dtypes('object').columns)] = data_predict[
        list(data_predict.select_dtypes('object').columns)].fillna('Пропущенное значение')

    model_class = CatBoostClassifier(task_type='GPU',
